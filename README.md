...
Personal configuration migrated to upstream `748f67f49dd9fed47686d1d15e8566d2cba8ed35`.
See [MIGRATION.md](MIGRATION.md) for preserved customizations, tests, and local setup notes.
Use `vim.pack`, not `:Lazy`. Keep `nvim-pack-lock.json` in version control.
`AGENTS.md` is unchanged and still describes the old Lazy layout; the migration report documents the current layout.

The upstream installation guide follows. Its deletion/uninstall examples are not migration steps: keep existing plugin installations and the full backup.
