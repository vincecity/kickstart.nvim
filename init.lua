...
-- vim-sleuth, gitsigns, which-key, colorscheme, todo-comments, mini modules
-- ============================================================
do
  ...
  --
  -- Keep vim-sleuth for indentation detection instead of upstream's guess-indent.
  -- Vimscript plugins such as this one need no Lua setup call.
  ...
  gitsigns.setup {
    ...
    on_attach = function(bufnr)
      ...
      vim.keymap.set('n', '<leader>hb', gitsigns.blame_line, { desc = 'git [b]lame line', buf = bufnr })
      ...
      vim.keymap.set('n', '<leader>tD', gitsigns.toggle_deleted, { desc = '[T]oggle deleted lines', buffer = bufnr })
      -- Toggles
  ...
  require('todo-comments').setup {
    signs = false,
    keywords = {
      IMPORTANT = { icon = ' ', color = 'warning', alt = { 'CRITICAL', 'URGENT' } },
      TODO = { icon = ' ', color = 'error', alt = { 'todo' } },
  ...
  require('mini.ai').setup {
    -- Keep the original an/in textobjects rather than upstream's aa/ii.
    -- These intentionally take precedence over Neovim 0.12 incremental selection.
    mappings = {
      around_next = 'an',
      inside_next = 'in',
    },
...
do
  ...
  -- Servers, capabilities and Mason are configured in custom.plugins.lsps.
end
...
do
  ...
  require('blink.cmp').setup {
    ...
    completion = {
      ...
      accept = { auto_brackets = { enabled = true, blocked_filetypes = { 'TelescopePrompt', 'zig', 'odin', 'vim', 'c', 'cpp' } } },
      list = { selection = { preselect = false, auto_insert = false } },
    },

    sources = {
      default = { 'lazydev', 'copilot', 'lsp', 'snippets', 'path' },
      providers = {
    ...
    -- Noice already provides automatic signature help.
...
do
  ...
  --
  -- The custom entry point loads modules in an explicit dependency order.
  -- Add vim.pack.add() and real setup calls there, not Lazy specification tables.
